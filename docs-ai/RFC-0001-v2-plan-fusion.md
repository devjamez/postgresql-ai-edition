...
| **M2** | **Transparent** fusion: plain `WHERE quals ORDER BY emb <=> $1 LIMIT k` auto-optimized (no special function), and/or pushing the filter into the ANN traversal for better recall | planner/executor hooks; pgvector internals (`hnswscan.c`) for recall | **DONE (opt-in):** detection in the planner hook (`pg_ai_core_stats().fusion_candidates`) + **auto-apply** via `ExecutorStart_hook`: when `pg_ai_core.auto_fuse` is on, a detected filtered-ANN query gets `hnsw.iterative_scan = strict_order` set **transaction-locally** (`GUC_ACTION_LOCAL`) — auto-reverts at transaction end, verified no leak. Opt-in (default off) to keep blast radius minimal. **Remaining:** pushing the filter *inside* the HNSW graph walk for better recall = pgvector internals, contributor task. |
...
- **M2 auto-apply — SHIPPED (opt-in).** Transparently enabling `hnsw.iterative_scan` for detected filtered-ANN queries, via `ExecutorStart_hook` setting the GUC **transaction-locally** (`GUC_ACTION_LOCAL`) — it auto-reverts at transaction end, so no nest-level bookkeeping and no cross-query leak (simpler and safer than the `auto_explain` save/restore dance). Gated behind `pg_ai_core.auto_fuse` (default off) to keep blast radius minimal. Verified: transparent enable returns full top-k where the bounded scan under-returns, and `hnsw.iterative_scan` is back to off in the next transaction.
- **M2 recall (remaining) — deferred.** Pushing the relational filter *into* the HNSW graph walk (so the ANN traversal itself skips non-matching nodes, improving recall/cost vs. over-fetch) requires modifying pgvector's `hnswscan.c`. Contributor task.
- **M3 — deferred, with specifics (not hand-waved).** (a) Native types: pgvector already provides a first-class `vector` type; a native `AGENT`/`MEMORY` type is speculative until the agent engine matures — adding one now is redundant effort. (b) Filtered-ANN cost model: the `pg_ai_fusion` custom scan is currently a passthrough (fusion logic lives in `ai.filter_ann`/auto_fuse), so there is no distinct node cost to model yet. Both become meaningful only after the recall work above; team-scale C, documented as long-horizon.
